$(document).ready(function () {
    ...
    const gameMenuElement = $('.game-menu');
    ...
    $('.menu-toggle, .menu_leave').on('click', function (e) {
        ...
        gameMenuElement.toggleClass('opened');
    ...
    function preventScroll(e) {
        e = e || window.event;
        if(e.target.className.indexOf('choices') === -1) {
            e.preventDefault();
            e.stopPropagation();
            return false;
        }
    ...
    function popupToggle(targetElement) {
        if (targetElement.target && (targetElement.target.className === 'fade' || targetElement.target.className === 'popup_close')) {
    ...
    $('.go-to-cashbox-open-window').on('click', function (event) {
        event.preventDefault();

        openModal('.popup-cashbox')
    });
    $('.go-to-account-open-window').on('click', function (event) {
        event.preventDefault();

        openModal('.popup-account')
    });
    ...
    /* Popups */
    $('.go-to-success-popup').on('click', function (event) {
        event.preventDefault();
        openModal('.js-popup-success')
    });
    $('.go-to-error-popup').on('click', function (event) {
        event.preventDefault();
        openModal('.js-popup-error')
    });
    $('.go-to-warning-popup').on('click', function (event) {
        event.preventDefault();
        openModal('.js-popup-warning')
    });
    $('.go-to-info-popup').on('click', function (event) {
        event.preventDefault();
        openModal('.js-popup-info')
    });
